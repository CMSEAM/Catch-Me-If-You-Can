...
class Adafruit_DotStar_hacked {

 public:

    Adafruit_DotStar_hacked(uint16_t n, uint8_t o=DOTSTAR_BRG);
    Adafruit_DotStar_hacked(uint16_t n, uint8_t d, uint8_t c, uint8_t o=DOTSTAR_BRG);
   ~Adafruit_DotStar_hacked(void);
  void
    begin(void),                            // Prime pins/SPI for output
    clear(),                                // Set all pixel data to zero
    setBrightness(uint8_t),                 // Set global brightness 0-255
    setPixelColor(uint16_t n, uint8_t c),
